...
class MHAOptionPolicy(nn.Module):
    ...
    def sample_action(self, st, ct, fixed=False, tau=1.0):
        ...
        log_pas = logits.log_softmax(dim=-1)  # (bs, dim_a)
        #
        if torch.any(torch.isnan(log_pas)) or torch.any(torch.isinf(log_pas)):
            log_pas = torch.where(torch.isnan(log_pas) | torch.isinf(log_pas), torch.zeros_like(log_pas), log_pas)
        if torch.any(log_pas < 0):     
            log_pas = torch.where(log_pas < 0, torch.zeros_like(log_pas), log_pas)
        log_pas += 1e-6  
        #
    ...
    def sample_option(self, st, ct_1, fixed=False, tau=1.0):
        ...
        if torch.any(torch.isinf(log_tr)) or torch.any(torch.isnan(log_tr)):
                    log_tr = torch.where(torch.isinf(log_tr) | torch.isnan(log_tr), torch.zeros_like(log_tr), log_tr)

                
        log_tr = torch.clamp(log_tr, min=0)
